...
password=getpass.getpass() #b6d91f0b35105993a2830fd4083d43cd
...
def nxapi_api(device):
    endpoints="https://"+device 
    cmds=open("vlans.json")
    payload=json.load(cmds)
    response=requests.post(endpoints,data=json.dumps(payload), auth=(user,password))
